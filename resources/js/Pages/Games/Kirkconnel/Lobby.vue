...
<template>
    <AppLayout title="Kirkconnel — Lobby">
        ...
        <div class="max-w-screen-xl mx-auto px-8 md:px-16 py-8">
            ...
            <p class="text-white/50 mb-8">A conquest strategy game. Dominate all territories to win.</p>

            <div class="grid grid-cols-3 gap-8">
                <!-- Create game -->
                <div class="col-span-1 bg-secondary rounded-lg p-6">
                    <h2 class="font-bold uppercase text-sm text-white/60 mb-4">Create Game</h2>
                    <label class="block text-sm mb-1">Select Map</label>
                    <select v-model="selectedMap" class="w-full bg-primary/10 border border-white/10 rounded px-3 py-2 text-sm mb-4">
                ...
                <div class="col-span-2">
                    <h2 class="font-bold uppercase text-sm text-white/60 mb-4">Open Games</h2>
                    <div v-if="!games.length" class="text-white/40 text-sm">No open games. Create one!</div>
                    <div v-for="game in games" :key="game.id" class="bg-secondary rounded-lg p-4 mb-3 flex items-center justify-between">
                        <div>
